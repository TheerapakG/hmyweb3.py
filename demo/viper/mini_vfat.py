...
from demo.viper.viper import VIPER_CHEF_ADDR, viper_chef_call, w3

address = YOUR_ADDRESS_AS_HEXSTR

block_number = w3.hmy.block_number
multiplier = viper_chef_call.getMultiplier(block_number, block_number+1)
reward_per_block = viper_chef_call.REWARD_PER_BLOCK()
rewards_per_week = ((reward_per_block/10**18)*multiplier*604800)/2

pool_count = int(viper_chef_call.poolLength())
total_alloc_points = viper_chef_call.totalAllocPoint()
...
def get_pool_info(index):
    pool_info =  viper_chef_call.poolInfo(index)
    ...
    poolToken = get_hmy_token(pool_info.lpToken, VIPER_CHEF_ADDR)
    userInfo = viper_chef_call.userInfo(index, address)
    pendingRewardTokens = viper_chef_call.pendingReward(index, address)
...
reward_token_address = '0xEa589E93Ff18b1a1F1e9BaC7EF3E86Ab62addc79'
...
tokens = {token_address: get_hmy_token(token_address, VIPER_CHEF_ADDR) for token_address in token_addresses}
